...
void EscribirTemperatura( int16_t temperatura, char reglon, char offset )
{
	...
	if( LCD.filas == LCD_2_FILAS )
	{
		...
		error = EscribirMensajeLCD( RENGLON_MEDIO_SUPERIOR, 0, 7, pTexto );
	}
	else
		error = EscribirMensajeLCD( reglon, offset, 7, pTexto );
}
...
void mandarNibbleAltoLCD_Probador( char dato )
{
	...
	// Se escribe la parte alta de los datos sobre el puerto
	auxiliar = dato & MASCARA_DB7_DATO_PROBADOR;
	if( auxiliar == 0 )
		LCD_DB7 = 0;
	else
		LCD_DB7 = 1;

	auxiliar = dato & MASCARA_DB6_DATO_PROBADOR;
	if( auxiliar == 0 )
		LCD_DB6 = 0;
	else
		LCD_DB6 = 1;
	
	auxiliar = dato & MASCARA_DB5_DATO_PROBADOR;
	if( auxiliar == 0 )
		LCD_DB5 = 0;
	else
		LCD_DB5 = 1;
	
	auxiliar = dato & MASCARA_DB4_DATO_PROBADOR;
	if( auxiliar == 0 )
		LCD_DB4 = 0;
	else
		LCD_DB4 = 1;
...
void mandarNibbleBajoLCD_Probador( char dato )
{
	...
	// Se escribe la parte alta de los datos sobre el puerto
	auxiliar = dato & MASCARA_DB3_DATO_PROBADOR;
	if( auxiliar == 0 )
		LCD_DB7 = 0;
	else
		LCD_DB7 = 1;

	auxiliar = dato & MASCARA_DB2_DATO_PROBADOR;
	if( auxiliar == 0 )
		LCD_DB6 = 0;
	else
		LCD_DB6 = 1;
	
	auxiliar = dato & MASCARA_DB1_DATO_PROBADOR;
	if( auxiliar == 0 )
		LCD_DB5 = 0;
	else
		LCD_DB5 = 1;
	
	auxiliar = dato & MASCARA_DB0_DATO_PROBADOR;
	if( auxiliar == 0 )
		LCD_DB4 = 0;
	else
		LCD_DB4 = 1;
...
/* inicializarPuertoLCD										*/
/*  														*/
/*  Define los pines hacia el LCD como salidas y  pone 0	*/
/*  														*/
/*  Recibe: Nada											*/
/*  Devuelve: Nada											*/
/************************************************************/
...
void mensajeLCD_FuncionDelSensor ( char posicion )
{
	if( LCD.caracteres == LCD_8_CARACTERES )
	{
		// Primero se borra la pantalla ya que no se puede imprimir el conjunto entero
		BorrarLCD();
...
/* EscribirCifra												*/
/*  															*/
/*  Muestra la cifra suministrada en el LCD						*/
/*  															*/
/*  Recibe: La cifra a mostrar									*/
/*  Devuelve: Nada												*/
/****************************************************************/
void EscribirCifra( char cifra, char renglon, char offset )
{
	...
	// Se lo pasa a caracteres ASCII
	digito[0] = cifra + '0';
	
	// Se imprime el valor
	error = EscribirMensajeLCD( renglon, offset, 1, digito );
	
	// Se posiciona el cursor sobre la cifra escrita
	enviarInstruccion( INSTRUCCION_LCD_RENGLON_SUPERIOR | offset );
}



/****************************************************************/
/* EscribirNumeroDeCable										*/
/*  															*/
/*  Muestra el numero del cable en el LCD						*/
/*  															*/
/*  Recibe: El numero del cable a mostrar						*/
/*  Devuelve: Nada												*/
/****************************************************************/
void EscribirNumeroDeCable( char cable, char reglon, char offset )
{
	...
	// Se imprime el valor
	error = EscribirMensajeLCD( reglon, offset, 3, pTexto );
}
