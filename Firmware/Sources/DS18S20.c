...
char	totalDeSensores;
char	totalDeSensoresPrevios;
char 	totalDeSensoresEliminados;
char 	totalDeSensoresAgregados;
...
// sensores[3] es el que va al manillar
// sensores[4] es el que va al zocalo externo para pruebas
...
/*  														*/
/*  Para el caso de la prueba reducida, la direccion de los	*/
/*  sensores esta definida de forma fija con "#defines", ya	*/
/*  que se los considera fijos en la instalacion.			*/
/*  														*/
/*  Recibe:	Nada											*/
/*  Devuelve: Nada											*/
/************************************************************/
void InicializarSensoresProbador ( void )
{
	...
	sensores[SENSOR_SOLDADO_EN_PLACA].presente = TRUE;
	...
	sensores[SENSOR_PARA_LA_MANGUERA].presente = TRUE;
	...
	sensores[SENSOR_PARA_LADO_CALIENTE].presente = TRUE;
...
/*  Devuelve: Nada											*/
/************************************************************/
void InicializarSensoresProbadorPruebaReducida ( void )
{
	// Direccion del sensor soldado a la placa
	sensores[SENSOR_SOLDADO_EN_PLACA].ROM[ 0 ] = DS18S20_SOLDADO_BYTE_0;
	sensores[SENSOR_SOLDADO_EN_PLACA].ROM[ 1 ] = DS18S20_SOLDADO_BYTE_1;
...
/*  														*/
/*  Toma el valor de temperatura de los 3 sensores fijos en	*/
...
char TomarTemperaturaSensoresPruebaReducida( void )
{
	...
	// Se verifica que se pueda leer la temperatura la temperatura del sensor para la manguera
	if( LeerTemperaturaDS18S20( SENSOR_PARA_LA_MANGUERA ) != SIN_ERROR )
		return( SENSOR_PARA_LA_MANGUERA + 1 );
	...
	// Al llegar a este punto, los 3 sensores estan correctos
	return( 0 );
}
...
void agregarRom( posicionSensores indiceSensor )
{
	...
	// Luego de agregarlo a la lista, se incrementa el contador de sensores registrados
	totalDeSensores++;
}
...
/************************************************************/
/* registrarCambiosEnUbicaciones							*/
/*  														*/
/*  Llamada desde "ActualizarListadoDeSensores" luego de	*/
/*  una actualizacion de los sensores conectados.			*/
/*  Contabiliza la cantidad de sensores agregados y/o 		*/
/*  quitados del bus.										*/
/*  														*/
/*  Recibe: Nada											*/
/*  Devuelve: Nada											*/
/************************************************************/
void registrarCambiosEnUbicaciones( void )
{
	// Variables auxiliares
	char indiceSensor;
	char indiceSensorPostEnumeracion;
	char indiceByte;
	
	// Se resetean los contadores
	totalDeSensoresEliminados = 0;
	totalDeSensoresAgregados = 0;
	
	// Primero se registra si se elimino algun sensor
	for( indiceSensor = 0; indiceSensor < TOTAL_DE_SENSORES; indiceSensor++ )
	{
		// Se verifica que la ROM no sea nula
		if( sensores[indiceSensor].ROM[BYTES_TIPO_DE_SENSOR] != 0 )
		{
			// Loop para recorrer los sensores posteriores a la enumeracion
			for( indiceSensorPostEnumeracion = 0; indiceSensorPostEnumeracion < TOTAL_DE_SENSORES; indiceSensorPostEnumeracion++ )
			{
				// Se verifica que la ROM no sea nula
				if( sensoresPostEnumeracion[indiceSensorPostEnumeracion].ROM[BYTES_TIPO_DE_SENSOR] != 0 )
				{
					// Loop para revisar todos los bytes de la ROM
					for( indiceByte = 0; indiceByte < LONGITUD_ROM_BYTES; indiceByte++ )
					{
						// Se verifica si alguno de los bytes de la ROM es distinto
						if( sensores[indiceSensor].ROM[indiceByte] != sensoresPostEnumeracion[indiceSensorPostEnumeracion].ROM[indiceByte] )
							break;			// Se corta el loop si difieren
					}
					
					// Finalizado el loop, si "indiceByte == LONGITUD_ROM_BYTES", es porque la ROM coincidia
					// La condicion "sensores[indiceSensor].ROM[BYTES_TIPO_DE_SENSOR] != 0" es para evitar las ROMS nulas
					if( indiceByte == LONGITUD_ROM_BYTES )
						break;
				}
			}
		
			// Si el loop termina por "indiceSensorPostEnumeracion == TOTAL_DE_SENSORES", es porque el sensor se elimino
			if( indiceSensorPostEnumeracion == TOTAL_DE_SENSORES )
			{
				// Se indica que no se va a usar mas
				sensores[indiceSensor].presente = FALSE;
				
				// Se borra la ROM almacenada
				for( indiceByte = 0; indiceByte < LONGITUD_ROM_BYTES; indiceByte++ )
					sensores[indiceSensor].ROM[indiceByte] = 0;
				
				// Se incrementa el contador de sensores eliminados
				totalDeSensoresEliminados++;
			}

			// Caso contrario, ya esta registrado, con lo que se lo borra de la nueva lista
			else
				for( indiceByte = 0; indiceByte < LONGITUD_ROM_BYTES; indiceByte++ )
					sensoresPostEnumeracion[indiceSensorPostEnumeracion].ROM[indiceByte] = 0;
		
		}
	}
	
	// Luego se realiza el agregado de los nuevos sensores en las posiciones libres
	for( indiceSensorPostEnumeracion = 0; indiceSensorPostEnumeracion < TOTAL_DE_SENSORES; indiceSensorPostEnumeracion++ )
	{
		// Se verifica que la ROM no sea nula
		if( sensoresPostEnumeracion[indiceSensorPostEnumeracion].ROM[BYTES_TIPO_DE_SENSOR] != 0 )
		{
			// Se revisa que posiciones libreas hay en el listado de sensores previos
			for( indiceSensor = 0; indiceSensor < TOTAL_DE_SENSORES; indiceSensor++ )
			{
				// Si la ubicacion esta libre, se agrega la nueva ROM
				if( sensores[indiceSensor].presente == FALSE )
				{
					// Se indica que a partir de ahora esta en uso
					sensores[indiceSensor].presente = TRUE;
					
					// Se copian los datos
					for( indiceByte = 0; indiceByte < LONGITUD_ROM_BYTES; indiceByte++ )
					{
						sensores[indiceSensor].ROM[indiceByte] = sensoresPostEnumeracion[indiceSensorPostEnumeracion].ROM[indiceByte];
						sensoresPostEnumeracion[indiceSensorPostEnumeracion].ROM[indiceByte] = 0;
					}
					
					// Se incrementa el contador
					totalDeSensoresAgregados++;
					
					// Se termina el loop para pasar al siguiente sensor
					break;
				}
			}
		}
...
bool reordenarSensores( void )
{
	...
	sensores[ SENSOR_SOLDADO_EN_PLACA ].presente = FALSE;		// Se borra el indicador, que sera renovado si se lo encuentra
	for( indiceSensorPost = 0; indiceSensorPost < totalDeSensores; indiceSensorPost++ )
	{
	...
	if( indiceSensorPost == totalDeSensores )
		error = TRUE;
	
	
	
	// Sensor para el lado caliente
	sensores[ SENSOR_PARA_LADO_CALIENTE ].presente = FALSE;		// Se borra el indicador, que sera renovado si se lo encuentra
	for( indiceSensorPost = 0; indiceSensorPost < totalDeSensores; indiceSensorPost++ )
	{
		// Si los datos de la ubicacion no se corresponden con algo valido, no se los toma en cuenta
		if( sensoresPostEnumeracion[ indiceSensorPost ].ROM[ BYTES_TIPO_DE_SENSOR ] == 0x10 )
	...
	sensores[ SENSOR_PARA_LA_MANGUERA ].presente = FALSE;		// Se borra el indicador, que sera renovado si se lo encuentra
	for( indiceSensorPost = 0; indiceSensorPost < totalDeSensores; indiceSensorPost++ )
	{
		// Si los datos de la ubicacion no se corresponden con algo valido, no se los toma en cuenta
		if( sensoresPostEnumeracion[ indiceSensorPost ].ROM[ BYTES_TIPO_DE_SENSOR ] == 0x10 )
	...
	sensores[ SENSOR_MANILLAR ].presente = FALSE;		// Se borra el indicador, que sera renovado si se lo encuentra
	for( indiceSensorPost = 0; indiceSensorPost < totalDeSensores; indiceSensorPost++ )
	{
		...
		if( sensoresPostEnumeracion[ indiceSensorPost ].ROM[ BYTES_TIPO_DE_SENSOR ] == 0x10 )
		{
			...
			for( indiceByteROM = 0; indiceByteROM < LONGITUD_ROM_BYTES; indiceByteROM++ )
			{
				sensores[ SENSOR_MANILLAR ].ROM[ indiceByteROM ] = sensoresPostEnumeracion[ indiceSensorPost ].ROM[ indiceByteROM ];
			}
			
			// Luego de almacenar el dato del nuevo sensor, se termina la funcion
			sensores[ SENSOR_MANILLAR ].presente = TRUE;
			break;
		}
	}
...
void moverDatosSensores ( char sensorOrigen, char sensorDestino )
{
	// Variable auxiliar
	char indiceByte;
	
	// Bucle para copiar los datos
	for( indiceByte = 0; indiceByte < LONGITUD_ROM_BYTES; indiceByte++ )
	{
		sensores[ sensorDestino ].ROM[ indiceByte ] = sensores[ sensorOrigen ].ROM[ indiceByte ];
		sensores[ sensorOrigen ].ROM[ indiceByte ] = 0;
	}

	sensores[ sensorDestino ].temperatura = 0;
	sensores[ sensorDestino ].presente = TRUE;
	sensores[ sensorOrigen ].temperatura = 0;
	sensores[ sensorOrigen ].presente = FALSE;
}
